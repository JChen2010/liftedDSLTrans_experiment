...
/**
	...
	* @param  mode  The mode: r for root to created, p for parent to created, e for existential
...
*/
	public static int genCreate(String pname, String cname, String ptype, String ctype, String aname, String root, String rtype, String rname, String mode, String roota)
	{
		try {
			...
			BufferedReader in = new BufferedReader(new FileReader("umlGenBaseC.dsltrans"));
			int lines = 0;
			String str;
			...
			in = new BufferedReader(new FileReader("umlGenBaseC.dsltrans"));
	...
	// Same as genCreate, but with the following extra arguments:
	// attr - The name of the attribute to be modified.
	// attrv - The value to set the modified attribute to.
	// Also, mode should not be "e" (it will still run, but will not modify anything)
	public static int genCreateModify(String pname, String cname, String ptype, String ctype, String aname, String root, String rtype, String rname, String mode, String roota, String attr, String attrv)
	{
		try {
			// Read in the base transformation
			...
			String s8 = "Layer3.xmi";
			String s9 = "baseRootAssociation";
			String s10 = "attributeBaseName";
			String s11 = "attributeBaseValue";
			...
			for (int i = start_index; i < end_index + 1; i++) {
				...
				// Replace attributeBaseName with attr
				if(input_arr[i].toLowerCase().contains(s10.toLowerCase())){
					input_arr[i] = input_arr[i].replace(s10, attr);
				}

				// Replace attributeBaseValue with attrv
				if(input_arr[i].toLowerCase().contains(s11.toLowerCase())){
					input_arr[i] = input_arr[i].replace(s11, attrv);
				}
			}
			...
			out.close();
			System.out.println("The generated modify create rule has been written to:\n" + rule);
	...
	public static void main(String[] args) {
		...
		*/
		...
		if(args[0].equals("c")) {
			...
			//Note - genModify is not updated for lifted DSLTrans. Use genCreateModify instead.
			//genModify(args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11]);
			genCreateModify(args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11], args[12]);
			executeRule(cwd + "/" + args[8] + ".dsltrans", cwd);
			System.out.println("The target model has been written to output.xmi.\n");
		}

		// Note - genMove is not used for lifted DSLTrans Call genCreate and genDelete from
		// executeExperiment instead.
		/*
