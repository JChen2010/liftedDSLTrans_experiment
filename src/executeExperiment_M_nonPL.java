...
public class executeExperiment {
	...
	public static void main(String[] args) {
		try {
			...
			// For move/modify, last argument indicates mode
			...
			// For Move/Modify mode 2, first parse input.xmi
			if (mode.equals("2")) {
				...
				int found_index = -1;
				int found_index_end = -1;
				String found_type = "";
				String split [];

				String search_str = "";
				for (int i = 0; i < input_arr.length; i++) {
					search_str = "Name=\"" + args[6] + "\"";
					if (input_arr[i].contains(search_str)) {
						found_index = i;
						if (input_arr[i].contains("/>")) {
							found_index_end = i;
						}
						split = input_arr[i].trim().split("\\s+");
						found_type = "</" + split[0].substring(1);
					}
					if (found_index != -1 && found_index_end == -1) {
						if (input_arr[i].contains(found_type)) {
							found_index_end = i;
						}
					}
				}
